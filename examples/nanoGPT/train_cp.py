...
from src.parallelism.context_parallel.cp_zigzag_backward import zigzag_attention
...
class CPPositionEmbedding(nn.Module):
    def __init__(self, wpe, pos_global):
        super().__init__()
...
torch.manual_seed(1337)   #Makind seed equal for all ranks
...
base_seed = 42
_batch_gen = torch.Generator()  # CPU generator, module level

def zigzag_split(t, cp_rank, cp_size):
    chunks = t.chunk(2 * cp_size, dim=1)
    return torch.cat([chunks[cp_rank], chunks[2 * cp_size - 1 - cp_rank]], dim=1)

def contiguous_split(t, cp_rank, cp_size):
    return t.chunk(cp_size, dim=1)[cp_rank]

def get_batch(split, iter_num):
    if split == 'train':
        data = np.memmap(os.path.join(data_dir, 'train.bin'), dtype=np.uint16, mode='r')
    else:
        data = np.memmap(os.path.join(data_dir, 'val.bin'), dtype=np.uint16, mode='r')
    ...
    seed = (base_seed
            + iter_num * 0x9E3779B1
            + (0 if split == 'train' else 0x5BF03635)) & 0x7FFFFFFFFFFFFFFF
    _batch_gen.manual_seed(seed)

    ix = torch.randint(len(data) - block_size, (batch_size,), generator=_batch_gen)
    ...
    if cp_size > 1:
        split_fn = zigzag_split if cp_mode == 'zigzag' else contiguous_split
        x = split_fn(x, cp_rank, cp_size)
        y = split_fn(y, cp_rank, cp_size)

    return x, y

class CPAttention(nn.Module):
    def __init__(self, attn, cp_group, cp_size, cp_rank):
        super().__init__()
        self.c_attn = attn.c_attn          # reuse the trained weights
        self.c_proj = attn.c_proj
        self.n_head = attn.n_head
        self.n_embd = attn.n_embd
        self.cp_group, self.cp_size, self.cp_rank = cp_group, cp_size, cp_rank
        self.send_to = (cp_rank + 1) % cp_size
        self.recv_from = (cp_rank - 1) % cp_size
    ...
    def forward(self, x):
        ...
        o_h, o_t = zigzag_attention(q_h, q_t, k_h, k_t, v_h, v_t,
                                    ...
                                    self.send_to, self.recv_from, self.cp_rank)

        y = torch.cat([o_h, o_t], dim=2)
...
_val_gen = torch.Generator().manual_seed(12345)
val_offsets = torch.randint(len(val_data) - block_size, (eval_iters * batch_size,),
                            generator=_val_gen)
...
def estimate_loss():
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            if split == 'val':
                ix = val_offsets[k * batch_size:(k + 1) * batch_size]
                ...
                if cp_size > 1:
                    split_fn = zigzag_split if cp_mode == 'zigzag' else contiguous_split
                    X, Y = split_fn(X, cp_rank, cp_size), split_fn(Y, cp_rank, cp_size)
            else:
                X, Y = get_batch(split, k)
            with ctx:
                logits, loss = model(X, Y)
            if cp_size > 1:
                dist.all_reduce(loss, op=dist.ReduceOp.AVG, group=cp_group)
            losses[k] = loss.item()
